...
import (
	...
	"github.com/rclone/rclone/fshttp"
	"github.com/rclone/rclone/fs/config/configmap"
	"github.com/rclone/rclone/fs/log"
	"github.com/rclone/rclone/fs/sync/pacer"
	"github.com/rclone/rclone/lib/rest"

	// 导入你的 tokenmanager 包，路径需要与你的 go.mod 模块路径一致
	"github.com/rclone/rclone/backend/123yunpan/tokenmanager"
)
...
func NewFs(ctx context.Context, name, root string, m configmap.Mapper) (fs.Fs, error) {
	...
	log.Debugf(nil, "[123CloudFs] Initializing TokenManager with URL: %s", cloudFunctionURL)
	tokenMgr := tokenmanager.NewManager(cloudFunctionURL, cloudFunctionAuthToken) // 传递新名称的参数

	// 3. 首次获取 token (使用 /get_token)
	log.Debugf(nil, "[123CloudFs] Attempting to get initial token from cloud function...")
	...
	log.Debugf(nil, "[123CloudFs] Token auto-refresh goroutine started.")

	// 5. 初始化你的 API 客户端
	log.Debugf(nil, "[123CloudFs] Initializing APIClient with BaseURL: %s", apiBaseURL)
	...
	log.Debugf(nil, "[123CloudFs] Backend initialized successfully.")
	return f, nil
}
	...
	for {
		...
		case <-ticker.C:
			...
			if initialSize > cleanedSize {
				log.Debugf(f, "[PathCache] Cleaned %d expired entries from cache. Current size: %d", initialSize-cleanedSize, cleanedSize)
			}
		case <-ctx.Done(): // 当 rclone 退出时，停止协程
			log.Debugf(f, "[PathCache] Cache cleaner stopped.")
			return
		}
	}
}
	...
	for i := range respData.Data.FileList {
		...
		if fileInfo.Type == 1 { // 只缓存目录
			...
			if !exists || time.Now().After(entry.expiresAt) {
				log.Debugf(f, "[PathCache] Caching/Updating path '%s' -> ID %d from listDir", remotePath, fileInfo.FileId)
...
func (f *Fs) pathToID(ctx context.Context, dirPath string) (int64, error) {
	...
	if found && time.Now().Before(entry.expiresAt) {
		log.Debugf(f, "[PathCache] Cache hit for path '%s' -> ID %d", dirPath, entry.id)
		return entry.id, nil
	}
...
func (f *Fs) List(ctx context.Context, dir string) (entries fs.DirEntries, err error) {
	log.Debugf(f, "[123CloudFs] Listing directory: %s", dir)
	...
	log.Debugf(f, "[123CloudFs] Path '%s' resolved to ID: %d", dir, parentID)
	...
	for {
		...
		for i := range respData.Data.FileList {
			...
			if fileInfo.Type == 1 { // 这是一个目录
				...
				if err != nil {
					log.Errorf(f, "Failed to parse directory mod time '%s' for %s: %v", fileInfo.UpdateAt, remotePath, err)
				...
				if err != nil {
					log.Errorf(f, "Failed to create object for %s: %v", remotePath, err)
					continue
				}
...
func (f *Fs) About(ctx context.Context) (*fs.Usage, error) {
	log.Debugf(nil, "[123CloudFs] Getting About information...")
	...
	log.Debugf(nil, "[123CloudFs] About successful: Total=%s, Used=%s", usage.Total, usage.Used)
	return usage, nil
}

// NewObject finds the Object at remote.
func (f *Fs) NewObject(ctx context.Context, remote string) (fs.Object, error) {
	log.Debugf(nil, "[123CloudFs] Getting object metadata for: %s", remote)
	return nil, fs.ErrorNotImplemented
}

// Mkdir makes the directory
func (f *Fs) Mkdir(ctx context.Context, dir string) error {
	log.Debugf(f, "Mkdir called for path: %s", dir)
	...
	if err != nil {
		log.Errorf(f, "Failed to find parent directory for Mkdir: %v", err)
	...
	if err == nil {
		log.Debugf(f, "Directory '%s' already exists, returning nil as per rclone spec.", dir)
		return nil // 目录已存在，Mkdir 应该成功返回
	}
	// 确保错误是 DirNotFound，否则可能是其他网络或API问题
	if err != fs.ErrorDirNotFound {
		log.Errorf(f, "Unexpected error during pre-existence check for Mkdir: %v", err)
		return err
	}
	...
	if respData.Code == 1 {
		log.Warningf(f, "Directory '%s' was created by another process concurrently. Treating as success.", dir)
	...
	f.pathCacheMu.Lock()
	log.Debugf(f, "[PathCache] Caching newly created directory '%s' -> ID %d", dir, newDirID)
...
func (f *Fs) getUploadDomain(ctx context.Context) (string, error) {
	...
	log.Debugf(f, "Upload domain cache is empty or expired, fetching a new one...")
	...
	log.Debugf(f, "Fetched and cached new upload domain: %s (expires in 10 minutes)", uploadDomain)
...
func (f *Fs) Put(ctx context.Context, in io.Reader, src fs.ObjectInfo, options ...fs.OpenOption) (fs.Object, error) {
	...
	if size < 0 || size > 200*1024*1024 { // 小于0或大于 200 MiB
		log.Debugf(src, "File size is %d, using chunked upload.", size)
		return f.putChunked(ctx, in, src, options...)
	}

	log.Debugf(src, "File size is %d, using single part upload.", size)
...
func (f *Fs) putChunked(ctx context.Context, in io.Reader, src fs.ObjectInfo, options ...fs.OpenOption) (fs.Object, error) {
	...
	if createRespData.Data.Reuse {
		log.Debugf(src, "Chunked upload completed via rapid upload (reuse).")
		fileInfo := &FileInfoV2{
	...
	for i := 0; i < numUploaders; i++ {
		...
		go func() {
			...
			for c := range chunkChan {
				...
				log.Debugf(src, "Successfully uploaded chunk #%d", c.number)
			}
		}()
	}
	...
	for {
		select {
		case <-ctx.Done(): // 轮询超时
			log.Warningf(src, "Polling for upload completion timed out after %v. Assuming success and returning an optimistic object.", pollTimeout)
		...
		if completeRespData.Data.Completed {
			...
			log.Debugf(src, "Upload completion confirmed. Final file ID: %d", finalFileID)
			goto end_poll
		}
...
func (f *Fs) trashItems(ctx context.Context, fileIDs []int64) error {
	...
	log.Debugf(f, "Trashed %d items, path cache cleared.", len(fileIDs))
...
func (f *Fs) clearPathCacheFor(remote string) {
	...
	log.Debugf(f, "Clearing path cache for remote '%s' and its children (prefix: '%s')", remote, prefix)
	...
	for k := range f.pathCache {
		...
		if k == remote || strings.HasPrefix(k, prefix) {
			...
			log.Debugf(f, "Cache entry removed: %s", k)
		}
	}
}
...
func (f *Fs) Rmdir(ctx context.Context, remote string) error {
	log.Debugf(f, "Rmdir on %s", remote)
...
func (f *Fs) Rename(ctx context.Context, o fs.Object, newName string) (fs.Object, error) {
	...
	log.Debugf(srcObj, "Renaming to '%s'", newName)
...
func (f *Fs) Move(ctx context.Context, src fs.Object, remote string) (fs.Object, error) {
	...
	// --- 情况 D: 无操作 ---
	if !isMove && !isRename {
		log.Debugf(src, "Move: source and destination are identical, doing nothing.")
		return src, nil
	}

	// --- 情况 A: 只移动 ---
	if isMove && !isRename {
		log.Debugf(src, "Moving from '%s' to '%s'", srcPath, dstPath)
	...
	// --- 情况 B: 只重命名 ---
	if !isMove && isRename {
		log.Debugf(src, "Renaming to '%s' in place", dstLeaf)
		return f.Rename(ctx, src, dstLeaf)
	}

	// --- 情况 C: 既移动又重命名 ---
	if isMove && isRename {
		log.Debugf(src, "Moving from '%s' to '%s' and renaming to '%s'", srcPath, dstPath, dstLeaf)
		...
		log.Debugf(src, "Move successful, now renaming to '%s'", dstLeaf)
		return f.Rename(ctx, src, dstLeaf)
	}
...
func (f *Fs) open(ctx context.Context, o *Object, options ...fs.OpenOption) (io.ReadCloser, error) {
	// --- 步骤 1: 获取下载 URL ---
	log.Debugf(o, "Opening file, requesting download URL")
	...
	log.Debugf(o, "Got download URL: %s", downloadURL)
...
var (
    ...
    _ fs.Copier  = (*Fs)(nil) // 当我们实现 Copy 时，会添加这一行
