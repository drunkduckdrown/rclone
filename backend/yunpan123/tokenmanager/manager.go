...
import (
	...
	//"github.com/rclone/rclone/fs/log" // rclone 提供的日志接口
...
func (m *Manager) callCloudFunction(endpoint string) (*TokenResponse, error) {
	...
	log.Debugf(nil, "[TokenManager] Calling cloud function: %s", fullURL)
...
func (m *Manager) GetAndStoreToken(endpoint string) error {
	...
	log.Debugf(nil, "[TokenManager] Token updated successfully from %s, expires at %s (actual: %s)",
...
func (m *Manager) StartAutoRefresh(ctx context.Context) {
	...
	log.Debugf(nil, "[TokenManager] Auto-refresh goroutine started.")

	for {
		select {
		case <-ticker.C:
			if m.IsTokenExpiredSoon() {
				log.Debugf(nil, "[TokenManager] Token expiring soon or not set, attempting auto-refresh via /get_token...")
				if err := m.GetAndStoreToken("/get_token"); err != nil {
					log.Errorf(nil, "[TokenManager] Auto-refresh failed: %v", err)
				}
			}
		case <-m.stopChan:
			log.Debugf(nil, "[TokenManager] Auto-refresh stopped via stopChan.")
			return
		case <-ctx.Done(): // rclone context 取消时停止
			log.Debugf(nil, "[TokenManager] Context cancelled, stopping auto-refresh.")
			return
		}
	}
}
